...
- **The chat has one look.** Until now it had three: a legacy boxed `loop` mode, the dark-canvas `noir` mode, and a *live variant* of noir that `ctrl+e` flipped on — which changed the rendering (runs of tool calls folded, the hints changed their wording) and swapped the transcript for a windowed viewport it scrolled itself. The same conversation therefore drew itself three different ways, and merely reaching for the keyboard re-laid-out the screen around you. All of it is gone. The UI-mode system — the registry, the twenty-knob style spec, the per-block renderer dispatch — is deleted, and there is simply the transcript: folded runs, a receipt and a short peek under each call, `❯` on your messages, thinking as a block that collapses when the turn moves on.
- **Folding works the way grok-build's does.** Its fold pass is ported rule for rule (`scrollback/state/groups.rs`, `verb_group.rs`), which fixes everything that made loop's folds feel wrong. A run of calls is **one row** — `◈ Read 3 files, Ran 2 commands, Edited 1 file` — where it used to be a header with a table of its members hanging off it. Opening a run keeps its header above the calls instead of making it vanish. Thinking is never folded: every thought keeps its own `◆ Thought for 2s` row and ends the run around it. **Opening one call** keeps its rows without dissolving the group around it. And a call that only *looks* (a read, a search, a listing, a subagent) **folds while it is still running**, the header counting it in the present tense ("Reading 3 files…") so the transcript holds still instead of growing a row per call and then collapsing them all at once.
- **Every tool folds once it finishes — including commands and edits.** grok keeps commands and edits as rows; loop folds them too, and keeps grok's distinction for exactly one moment: while a command or an edit is **running**, it keeps its own row so its live output can be watched, and it joins the run the moment it lands. Plans are the one exception and never fold.
- **Navigation is focus, and nothing else.** `ctrl+e` hands the keyboard to the transcript, and the frame no longer moves when it does: the rows are the same rows, in the same place on the page. The selection bar appears, the arrows start meaning "entry" instead of "history", and the hint row says so. The selected fold's glyph turns into a chevron that says which way `→` and `←` will take it. Scrolling belongs to the page, so the wheel, `PgUp`/`PgDn`, `ctrl+u`/`ctrl+d` and `Home`/`End` behave the same whether or not the transcript has the keyboard, and a selected entry that is off-screen is scrolled to rather than framed by a second viewport.
- **Clicks select entries while you navigate.** Inside `ctrl+e`, clicking a row, a message or a thought selects it. Outside it a click is the terminal's business and changes nothing, so pointing at the screen while you type can never take the prompt out from under you. The click is recognised where text selection already is — a press and release on one cell, no drag — so dragging still selects text exactly as before, and loop no longer grabs and releases mouse reporting as you enter and leave navigation.
- **Esc puts everything back.** Leaving navigation returns the transcript to exactly the state it was in before you entered: groups closed, calls and thoughts folded. Only a `full` density chosen with `d` is kept, since that is a setting rather than something you opened.
- **Themes are the only thing left to choose, and they wear grok's canvas.** `night` and `day` take GrokNight and GrokDay's neutral grayscale ramp and semantic accents from grok-build, while markdown — headings, inline code, code blocks, links, syntax — keeps loop's own softer colours, and `system` still rebuilds that ink for whatever background your terminal already has. One settings key, `theme`, replaces `uiMode`, `uiLive` and `uiThemes`; an old settings.json is read as it was meant (`theme`, then `uiThemes.noir`, with the retired `dark`/`light` resolving to `night`/`day`), so nothing has to be edited. `/theme` shows the picker or switches straight to a name; `/ui` is gone.

### Fixed

- **The TUI no longer freezes when one thing fails to draw.** A component that threw while rendering escaped the whole frame; the process's last-resort error handler showed the error in the chat and asked for a repaint, and the repaint threw again. Nothing ever painted — typing and scrolling looked dead — while the chat grew by an error line per frame, until a restart rebuilt the components from the session and it "worked" again. Now a transcript entry that cannot be drawn draws one line saying so and everything around it keeps working; a frame that fails anyway is logged once (`render-error.log` in the agent directory, with the stack) and never re-requests itself, so the UI recovers the moment the fault clears; the exit print is guarded the same way, so quitting always restores the terminal; and the error handler counts a repeating error instead of appending it forever.
- **Errors appear where they happened, and a failed turn says so once.** A turn streams into its own block, and every error, hook note and system line was appended below that block — so anything raised mid-turn stuck to the bottom of the screen while the rest of the turn streamed in above it. They now land in arrival order, as grok keeps them. And a turn that fails closes with one line, `Turn failed in 2s: <why>` (grok's `TurnFailed`), where it used to print `error: …` followed by a "Turn completed in 2s." that contradicted it; a stream error and the exception that carries it out of the turn are one failure, not two lines.
- **`system` no longer paints a dark terminal with the light set.** It asked the terminal for its colour-scheme report first and its background colour second, each with 250ms to answer. On a busy machine the colour lost that race, the report won — and on macOS the report is the OS appearance, not the terminal's — so a dark terminal on a light desktop came up with near-white message boxes and dark text. The background colour is now asked first, with room to answer, and on macOS the report alone never switches to the light set.
- **Twelve old sessions could not be reopened.** Sessions from older versions store bare model ids (`grok-4`) or none, and replay parsed them as if asked to call a model — `/resume` said "open failed", and `loop --session` crashed at startup on a fresh install with no model selected. Replay reads history now, whatever model it names.
- **`loop --session <id>` comes up with the right context gauge.** Resuming from the command line showed an empty `ctx` meter until the first turn, while `/resume` inside a session filled it in. The model's context window is read from the catalog, which at startup has not been built yet, so a custom-provider or gateway id resolved to nothing and the gauge got a maximum of zero; the catalog landed a moment later and re-applied only the model. The whole status line is now re-applied when it lands, which also restores the cost row and the plan-mode flag on that path.
- **An interrupted tool call no longer disappears into a fold.** It produced no result, so folded into a count it read as "finished, returned nothing" when what happened was that you stopped it. It keeps its own row, saying `· interrupted`.
- **A folded subagent says what it was** — `explore · look around`, not the raw JSON of its arguments.

### Removed

- **The `loop` UI mode, its `dark`/`light` palettes, and the `/ui` command.** Settings naming them keep working — they resolve to the transcript's own look and the nearest theme.
- **`api.uiModes` (extension SDK break).** With one transcript there is nothing for a second one to be. Palettes are still contributed, through `api.themes.register(...themes)`, and a partial palette is filled in from `night`. No bundled extension used the old surface.
...
- **MCP is no longer tools-only.** A server's resources, prompts and elicitations were unreachable however loudly it advertised them, because loop only ever called `tools/list`. Resources are now discovered per server and reachable two ways: the agent reads them with the `mcp_resource` tool (list, or read by uri; binary contents are described rather than inlined as base64, which is unreadable and costs a context window), and you point at one in the composer with `@mcp:<server>:<uri>` — typing `@mcp:` completes them, while a bare `@` stays exactly as file-only and as fast as before. Server prompts become real slash commands, `/mcp:<server>:<prompt>`, registered as servers connect and re-synced when one announces `prompts/list_changed`, so a server that goes away takes its commands with it. Their arguments accept `name=value` in any order or bare positional values, and are completed **by the server** through `completion/complete` — the menu offers the branch names or paths that argument actually means. Elicitation is answered rather than refused: a server that asks a question mid-tool-call gets loop's selector or prompt, with the server's name on every screen, and typed values back (a number field returns a number, not the string that was typed). Everything is gated on the capabilities the server declared in its handshake, so an ordinary tools-only server is never asked a question it would answer with a method-not-found.
- **A server's own instructions reach the model.** The `initialize` result carries an `instructions` field — it is how a server explains its own conventions — and loop discarded it for every server it has ever connected to. It is now included in the system prompt, attributed to the server it came from and scoped to that server's tools, because two servers' instructions are not interchangeable and an unattributed blob reads as loop's own rules.
- **Per-server tool policy.** `allowedTools` / `deniedTools` in a server's config filter what it contributes, matched against the server's own tool names rather than loop's namespaced keys (which are shortened when long, so matching them would break on exactly the verbose servers a policy gets written for). A denied tool is never advertised at all, so it costs no context and the model never plans around it. Deny beats allow, and an empty allow list means none.
- **Tool search for large MCP setups.** Every advertised tool's name, description and schema rides every request, so a couple of chatty servers can outweigh loop's entire builtin toolset before a file has been read. Above a threshold — 50 tools by default, set `mcpToolSearch` to a number, `true` or `false` — the individual tools stop being advertised and the model reaches them through one `mcp_tools` tool that searches, describes and calls them. The system prompt says so explicitly, because a model shown one unfamiliar tool where it expected a server's tools concludes the capability is missing rather than searching for it.
- **`/mcp` says what a server actually contributes,** counting resources and prompts beside tools, and gains `inspect` (declared capabilities, resources, prompts, instructions, and any tool policy) and `check health` per server.

### Fixed

- **Any notification from an MCP server killed its connection.** The SDK has no notification handling: a message with a `method` and no `id` — which is every `tools/list_changed`, every log line, every progress report — was reported as an uncaught transport error, and loop read that as the connection dying. A server doing something completely ordinary was marked errored and had all of its tools withdrawn mid-session while the process behind it was still running fine. Notifications are now intercepted at the transport before the SDK can mistake them for errors, and a protocol message that is merely unhandled (an unimplemented `sampling` or `roots` request) is no longer treated as a dead connection either.
- **`tools/list_changed` now does what it says.** A server that adds, removes or renames a tool mid-session is re-listed, with withdrawn tools actually dropped rather than left in the turn's toolset for the model to keep calling. A burst of notifications — what a server rebuilding its registry emits — collapses into one refresh.
- **A server that is running but no longer answering is caught.** A dead stdio child announces itself through its transport; a remote server does not, so a silently dropped connection stayed "ready" and advertised its tools for the rest of the session while every call failed. Connected servers are now probed on a timer, each probe bounded by its own timeout — without which the probe inherits the failure it exists to detect and hangs forever. `LOOP_MCP_HEALTH_INTERVAL_MS=0` turns it off.
- **A server that drops reconnects on its own,** with backoff and a limited number of attempts, instead of sitting dead until someone notices and runs `/mcp reconnect`. Anything you do to the server yourself — disabling, deleting, reconnecting by hand — cancels the retry, so a server you just removed cannot come back.
- **The alternate screen repaints in full after a terminal resize.** A shrink can discard or shift rows even when a later resize restores the original size, which left the cached screen an unsafe baseline to diff against; the baseline is now dropped while layout and scroll position are kept, so input arriving before the repaint still targets the current view.

### Changed

- **The web UI adds a Trajectory view.** Switch from Chat to Trajectory above the conversation to inspect a session as a searchable event timeline, with Input, Model and Tools lanes, optional recorded durations with idle gaps compressed, drag-to-focus, zoom and pan. Missing durations and per-request token timings are not inferred.
- **The last of the t3code names are gone from the web app** — project-file contracts, the Connect sign-in components and the client storage keys now carry loop's own names, with a migration that moves existing browser state across rather than stranding it.
...
- **Mermaid code blocks are drawn as diagrams instead of printed as source.** Flowcharts (`flowchart`/`graph`, TD/BT/LR/RL) and sequence diagrams render in box drawing, with layered layout: ranks by longest path, cycles cut by DFS so a back edge routes around the graph rather than through a box, crossings reduced by barycentre sweeps, and coordinates set from the median of a node's real neighbours so a chain comes out straight. They draw **while the reply streams**, one settled line at a time — the line still being typed is held back, because a half-written `A[Requ` parses as a bare node and the box would flicker through its own label a character at a time. A `subgraph` whose members have no edges between them folds into one titled box listing them, which is what a cluster of unconnected nodes is, and is far narrower than the same nodes strung across a rank. When a diagram will not fit, labels wrap first and only then is the diagram turned on its side: a terminal is bounded across and unbounded down, so a fan-out too wide as TD draws as LR, and turning it costs less than breaking `@notshekhar/loop` across two lines. Anything not supported — a diagram kind, a malformed source, or one still too wide at the tightest wrapping — falls back to the fenced source, unchanged.
- **`/recipe` saves a finished session as a reusable workflow.** `/recipe save <name>` asks what should vary, extracts the method as Markdown with `{{input}}` placeholders, and shows an editable draft before writing anything. Recipes are plain files under `~/.loop/recipes/`, so editing one by hand is a first-class way to change its inputs. `/recipe <name> resource=users` binds by name or position and prompts for whatever is missing; substitution is a single pass, so a value containing `{{other}}` stays literal. The rendered body is submitted as an ordinary turn — a recipe is a prompt, not a script, and nothing in it is evaluated as shell.
- **`/handoff` continues the current task in a fresh session.** It drafts an editable brief — objective, decisions, changed files, verification, failed approaches, next steps — from the conversation and a `git status` snapshot, then lets you pick the destination model and agent before opening. The new session is written in one transaction with the brief as its opening message and a link back to the source; source entries are never copied or mutated, so the original stays live and resumable. The brief's prompt is told the transcript is evidence rather than instructions, not to infer that a command passed without evidence for it, and not to attribute every dirty file in the workspace to the agent.

### Fixed

- **`/handoff` and `/recipe save` no longer cap their own output at 4,000 tokens.** On a reasoning model that budget is shared with the reasoning, and on a long session the reasoning spent all of it: a real run recorded `input=15899 output=4000 reasoning=4000` and returned no text at all, so `/handoff` failed on exactly the sessions long enough to want handing off. Both now take the model's real ceiling from the catalog, as every other call in core already did — 384,000 output tokens for the model in that run rather than 4,000. Simply dropping the field would not have fixed it: the AI SDK falls back to 4,096 for a model id it does not recognise, which is the same wall.
- **A response cut off mid-reasoning says so.** The empty-output check ran before the truncation check, so a run that spent its whole budget thinking reported "the model returned an empty handoff, try again" — the one instruction that cannot help, because the next attempt truncates in the same place. Truncation is reported first now, in both commands.
- **A handoff brief and a recipe draft are ruled off from the conversation.** Both rendered as bare Markdown, indistinguishable from an assistant turn, while being neither: they are drafts for a session that does not exist yet and never enter the current session's context. They now carry a header and borders like every other transient block, and say so. `/recipe show` went through the same path and printed a stored recipe as though the model had just written it.
