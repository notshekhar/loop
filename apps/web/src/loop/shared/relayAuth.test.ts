...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.loop.example.com"))).toBe(
      "clerk.loop.example.com",
    );
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.loop.example.com"))).toBe(
      "https://clerk.loop.example.com",
    );
  });
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.loop.example.com/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.loop.example.com", "clerk.loop.example.com")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.loop.example.com")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.loop.example.com", "clerk.loop.example.com")).toBe(
      false,
    );
  });
});
