...
/**
 * Drives the primary environment's Loop Connect link. Loop Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update Loop Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[loop-connect] Could not update Loop Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Loop Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Loop Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Loop Connect before enabling this."));
        return false;
      }
