...
export function ResourceTelemetryDiagnostics() {
  ...
  const allLoop = snapshot?.groups.allLoop;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allLoop ? `${allLoop.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={allLoop ? `${formatCpuTime(allLoop.cpuTimeMs)} observed CPU time` : undefined}
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allLoop ? formatBytes(allLoop.currentRssBytes) : "..."}
              detail={
                allLoop ? `${formatBytes(allLoop.peakRssBytes)} combined process peaks` : undefined
              }
            />
            <IconStat
              icon={<ActivityIcon className="size-3.5" />}
              label="Process count"
              value={allLoop ? String(allLoop.processCount) : "..."}
              detail={
                allLoop ? `${allLoop.processStarts} starts · ${allLoop.processExits} exits` : undefined
              }
            />
            <IconStat
              icon={<HardDriveIcon className="size-3.5" />}
              label="Read throughput"
              value={allLoop ? formatRate(allLoop.ioReadBytesPerSecond) : "..."}
              detail={allLoop ? `${formatBytes(allLoop.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allLoop ? formatRate(allLoop.ioWriteBytesPerSecond) : "..."}
              detail={allLoop ? `${formatBytes(allLoop.ioWriteBytes)} observed` : undefined}
              tone={
                allLoop && allLoop.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allLoop && allLoop.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Loop operations so process spikes can be correlated with specific
