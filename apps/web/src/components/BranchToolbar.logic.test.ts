...
describe("resolvePreviousWorktreeSeed", () => {
  it("picks the most recently updated worktree thread", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "loop/older",
            worktreePath: "/repo/.loop/worktrees/older",
            updatedAt: "2026-07-20T00:00:00.000Z",
          },
          {
            branch: "loop/newer",
            worktreePath: "/repo/.loop/worktrees/newer",
    ...
    ).toEqual({ branch: "loop/newer", worktreePath: "/repo/.loop/worktrees/newer" });
  });
    ...
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "loop/current",
            worktreePath: "/repo/.loop/worktrees/current",
            updatedAt: "2026-07-22T00:00:00.000Z",
          },
        ],
        currentWorktreePath: "/repo/.loop/worktrees/current",
      }),
    ).toBeNull();
  });
  ...
  it("ignores archived threads and threads with unparseable timestamps", () => {
    expect(
      resolvePreviousWorktreeSeed({
        threads: [
          {
            branch: "loop/archived",
            worktreePath: "/repo/.loop/worktrees/archived",
            updatedAt: "2026-07-23T00:00:00.000Z",
            archivedAt: "2026-07-23T01:00:00.000Z",
          },
          {
            branch: "loop/garbage-timestamp",
            worktreePath: "/repo/.loop/worktrees/garbage",
            updatedAt: "not-a-date",
          },
          {
            branch: "loop/live",
            worktreePath: "/repo/.loop/worktrees/live",
    ...
    ).toEqual({ branch: "loop/live", worktreePath: "/repo/.loop/worktrees/live" });
  });
});

describe("resolvePreviousWorktreeLabel", () => {
  it("includes the branch when known", () => {
    expect(resolvePreviousWorktreeLabel({ branch: "loop/fix-thing", worktreePath: "/wt" })).toBe(
      "Previous worktree (loop/fix-thing)",
    );
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.loop/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a ref already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.loop/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveLocalCheckoutBranchMismatch", () => {
  ...
  it("ignores dedicated worktrees because their checkout is already thread-scoped", () => {
    expect(
      resolveLocalCheckoutBranchMismatch({
        effectiveEnvMode: "worktree",
        activeWorktreePath: "/repo/.loop/worktrees/feature-thread",
        activeThreadBranch: "feature/thread",
        currentGitBranch: "feature/current",
      }),
    ).toBeNull();
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.loop/worktrees/feature-a",
...
describe("resolveCurrentWorkspaceLabel", () => {
  ...
  it("describes the active checkout as a worktree when one is attached", () => {
    expect(resolveCurrentWorkspaceLabel("/repo/.loop/worktrees/feature-a")).toBe("Current worktree");
  });
});
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.loop/worktrees/feature-a")).toBe("Worktree");
  });
});
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected ref", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        refName: {
          isDefault: false,
          worktreePath: "/repo/.loop/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.loop/worktrees/feature-b",
      nextWorktreePath: "/repo/.loop/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default refs", () => {
    expect(
      ...
      checkoutCwd: "/repo/.loop/worktrees/feature-a",
      nextWorktreePath: "/repo/.loop/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
